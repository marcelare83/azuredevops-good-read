...
So the paths to the test results is implicitly set (I think it relies on them being in `$(Agent.TempDirectory)/TestResults` which is the default folder) AND the binary `.coverage` format is automatically converted to XML. This to me is way to much magic and a bad way to do things... Either way, if you are not running on a Windows image (which you [should avoid for performance reasons](#-run-on-linux-based-agents-when-possible))
